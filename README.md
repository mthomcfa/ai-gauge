...
Current version: **1.3.1+cfa.6** — a fork version, see [Versioning](#versioning). Release notes in [CHANGELOG.md](CHANGELOG.md).
...
`<file-version>` is the version with `+` replaced by `-`, so `1.3.1+cfa.6` ships as `ai-gauge-1.3.1-cfa.6-windows.zip`. Print it with `python tools/check_versions.py`.
