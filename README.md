...
Current version: **1.3.0+cfa.5** — a fork version, see [Versioning](#versioning). Release notes in [CHANGELOG.md](CHANGELOG.md).
...
`<file-version>` is the version with `+` replaced by `-`, so `1.3.0+cfa.5` ships as `ai-gauge-1.3.0-cfa.5-windows.zip`. Print it with `python tools/check_versions.py`.
