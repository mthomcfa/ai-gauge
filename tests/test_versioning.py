...
def test_packaging_steps_never_use_the_raw_version(step_name):
    ...
    assert "outputs.file_version" in run
    # Substring check would match file_version too, so match the exact ref.
    assert "outputs.version" not in run.replace("outputs.file_version", "")
