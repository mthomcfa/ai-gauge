...
def _collides_with_known(
    ...
    * a candidate that is something known plus trailing junk and no new word
    ...
    elided ``Opus only…`` is what a real page produces without trying. What
    surrounds it makes no difference: a row carrying words of its own shields
    the label inside it exactly as a decorated one does. Exact membership is
    tested beside the containment, because a label whose own edges are
    punctuation is not a whole-word match for *itself* and must still shield
    its own fragments.
    """
    normalized = normalize_label(label)
    if not normalized:
    ...
    for other in known:
        ...
        if _overlaps(normalized, other) and (
            rendered is None
            or other in rendered
            or any(_overlaps(other, r) for r in rendered)
        ):
            return True
...
def adopt_rows(
    ...
    *,
    ...
    #
    # `clean_label` tidies nine characters and no more, which is why
